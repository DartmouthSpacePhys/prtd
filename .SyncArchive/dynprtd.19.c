...
/**************** read_new_samples() *************************/
void read_new_samples(unsigned short *samples, struct parsed_options *options) {
	...
	do {
		...
		fread(&header, sizeof(header), 1, in); // Read in header

	} while ( ((old_sec == header.start_timeval.tv_sec && old_usec
			== header.start_timeval.tv_usec)) && (fclose(in) == 0) && (usleep(1e4) == 0) );
...
/*********** fft_new_samples() ***********************/
void fft_new_samples(unsigned short *samples) {
	int i;
