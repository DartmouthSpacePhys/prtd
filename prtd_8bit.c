/*
 *  prtd: process real-time 8-bit data coming from Ch10 data files
 */
...
void read_new_samples(struct parsed_options *o) {
	...
	if (samples != NULL)
		free(samples);
	samples = malloc( header .num_read);
	//fprintf(stderr,"\nReading Samples (%lu bytes) ... ",header.num_read);
	fread(samples, header .num_read, 1, in);
	//fprintf(stderr,"Done.\n");
