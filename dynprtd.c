...
int main(int argc, char **argv) {
	...
	long int procid;
	char outstring1[100];
	FILE *monfile, *image1, *image2, *image3, *image4, *in, *out;
	static unsigned long old_sec = 0, old_usec = 0;
	struct header_info header;
	int sz_samp = sizeof(unsigned short);
	int sz_fftd = sizeof(double);
	...
	while (1) {
		...
		/*
		 * 	Check save times in header of monitor file.  Sleep between checks.
		 */
		do {
		
			monfile = fopen(options->monfile, "r");
			fread(&header, sizeof(header), 1, monfile); // Read in header

		/*
		 *	This only works because of the short-circuiting nature of &&.
		 *	The fclose and usleep will only be checked (i.e. run) if the times match.
		 */
		} while ( ((old_sec == header.start_timeval.tv_sec && 
				old_usec == header.start_timeval.tv_usec)) && 
				(fclose(monfile) == 0) && (usleep(10e4) == 0) );

		/*
		 * 	Read in the dataz
		 */
		samples = realloc(samples, 2*header.num_read);
	...
	//		printf("\nReading new samples");
		...
		if (!options.avg && total_samples > header.num_channels*options.height) {
			// If we're not averaging, we just want enough data for the first n_chan FFTs
			total_samples = header.num_channels*options.height;
		}
		
		int n_chan = header.num_channels;
		int n_samp = total_samples/n_chan;

		old_sec = header.start_timeval.tv_sec;
		old_usec = header.start_timeval.tv_usec;
	
		/*
		 *	Now we need to allocate memory for the data we just read in.  Good times, good times.
		 */
		 
		fft_samples = realloc(fft_samples, sz_samp * total_samples); // This is the actual data store
		fft_outputs = realloc(fft_outputs, sz_fftd * total_samples); // This is the actual data store
		fft_channels = realloc(fft_channels, sizeof(unsigned short *) * header.num_channels); // This is pointers to the individual channels
		fft_chanouts = realloc(fft_chanouts, sizeof(double *) * header.num_channels); // This is pointers to the individual channel outputs
		
		for (int i = 0; i < header.num_channels) {
			fft_channels[i] = (fft_samples + i*total_samples/header.num_channels);
			fft_chanouts[i] = (fft_outputs + i*total_samples/header.num_channels);
		}
